...
QString DialogCompare::printAddress(int address, int digits)
{
    ...
    {
        curDigit = 0x0f & (address >> (4 * i));
        if (curDigit < 10) symbol = static_cast<uint8_t>(curDigit + 0x30);
        else symbol = static_cast<uint8_t>(curDigit + 0x37);
        result = result + QString(QChar::fromLatin1(static_cast<char>(symbol)));
    }
    result = result + "> ";
    return result;
}

QString DialogCompare::bytePrint(unsigned char z)
{
    unsigned char s;
    s = z / 16;
    if (s > 0x9) s = s + 0x37;
...
QString DialogCompare::getCRC32(QByteArray buf)
{
    uint32_t size, i;
    ...
    size = static_cast<uint32_t>(buf.length());
    i = 0;
    while (size--)
            {
              crc = (crc >> 8) ^ Crc32Table[(crc ^ static_cast<uint8_t>(buf[i])) & 0xFF];
              i++;
            }
        return hexiAddr(crc ^ 0xFFFFFFFF);
}

QString DialogCompare::hexiAddr(uint32_t add)
{
   QString rez = "";
   uint8_t A,B,C,D;
   D = 0xFF & add;
